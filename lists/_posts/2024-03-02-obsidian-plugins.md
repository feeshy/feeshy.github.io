...
- [Image Converter](https://obsidian.md/plugins?id=image-converter) - 图片压缩转换、命名、路径管理工具。粘贴图片时自动转换为 WebP 等高压缩率格式并限制尺寸，解决无损 PNG 截图导致的仓库体积膨胀问题。
- [Attachment Management](https://obsidian.md/plugins?id=attachment-management) - 附件命名与路径管理。粘贴附件时自动重命名并分流存储；支持在笔记改名时，同步重命名对应附件并更新链接。
- [Unique Attachments](https://obsidian.md/plugins?id=unique-attachments) - 附件命名与去重工具。基于文件内容的 MD5 值自动重命名附件，将重复的附件合并并清理多余的物理文件。
- [File Cleaner Redux](https://obsidian.md/plugins?id=file-cleaner-redux) - 冗余文件清理器。扫描并批量删除未引用的孤立附件、0 字节空笔记及空文件夹；支持设定忽略规则并在删除前提供预览确认。

以上插件功能有一定重合，部分功能同时开启可能会有冲突。以下是重合功能对比：

**附件命名**

| 对比维度   | Image Converter                       | Attachment Management                       | Unique Attachments                        |
| :----- | :------------------------------------ | :------------------------------------------ | :---------------------------------------- |
| 支持文件格式 | 仅限图片                                  | 所有附件                                        | 所有附件                                      |
| 重命名规则  | **静态单次**：仅在粘贴并格式转换时重命名；不支持跟随笔记改名      | **动态关联**：支持笔记改名时同步重命名附件；支持时间戳、MD5、源文件名等多种规则 | **内容哈希**：仅支持基于文件内容的 MD5 值重命名              |
| 协同配置建议 | **关闭重命名**：关闭路径与重命名配置，仅保留 WebP 格式转换与压缩 | 与 Unique Attachments **二选一**，共存会产生命名冲突      | 与 Attachment Management **二选一**，共存会产生命名冲突 |

**孤儿与重复文件清理**

| 对比维度       | File Cleaner Redux                    | Unique Attachments                                   |
| :--------- | :------------------------------------ | :--------------------------------------------------- |
| 清理类型   | **孤儿文件**：未引用的附件、0 字节空笔记、空文件夹          | **重复文件**：内容相同但路径与名称不同的多余副本                           |
| 检测原理   | **引用检测**：检查附件与文件是否在任何 Markdown 笔记中被引用 | **哈希校验**：计算文件内容的 MD5 哈希值并自动合并重名                      |
| 防误删保护  | **支持**：提供路径与后缀排除过滤，且删除前提供预览列表确认       | **安全**：仅自动合并相同内容文件，不影响且不破坏任何笔记引用                     |
| 协同配置建议 | **推荐开启**：作为主力清理器；建议配置移至废纸篓并添加排除路径     | **二选一安装**：使用其强制 MD5 命名机制时，必须卸载 Attachment Management |
